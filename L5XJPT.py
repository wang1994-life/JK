...
date_dir = os.path.join(os.getcwd(), 'L5虚拟化平台')
...
class L5XJPT_run:
    ...
    def login_L5xnj(self):
        ...
        if element_xj:
            driver.save_screenshot('L5-虚拟机.png')
            # 加载图像
            image = Image.open('L5-虚拟机.png')
            # 获取图像的宽度和高度
            left = 390
            top = 20
        ...
        if element_store:
            driver.save_screenshot('L5-虚机存储.png')
            # 加载图像
            image = Image.open('L5-虚机存储.png')
            # 获取图像的宽度和高度
            left = 390
            top = 20
            # 设定你想要裁剪的区域，确保不超出原图的边界
            ...
            cropped_image = image.crop(crop_box)
            # 保存或处理裁剪后的图像
            cropped_image.save('L5-虚机存储.png')
        time.sleep(3)
        driver.close()
