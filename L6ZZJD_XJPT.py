...
options.add_argument("--force-device-scale-factor=0.92")
# 禁用密码保存弹窗
...
date_dir = os.path.join(os.getcwd(), 'L6ZZJD虚拟化平台')
...
class L6XJPT_run:
    ...
    def login_L6xnj(self):
        url = 'https://10.140.13.1/ui/'
        ...
        if element_xj:
            driver.save_screenshot('L6ZZJD-虚拟机.png')
            # 加载图像
            image = Image.open('L6ZZJD-虚拟机.png')
            # 获取图像的宽度和高度
            left = 390
            top = 20
            ...
            # 保存或处理裁剪后的图像
            cropped_image.save('L6ZZJD-虚拟机.png')
            time.sleep(5)
        ...
        if element_store:
            time.sleep(3)
            driver.save_screenshot('L6ZZJD-虚机存储.png')
            # 加载图像
            image = Image.open('L6ZZJD-虚机存储.png')
            # 获取图像的宽度和高度
            left = 390
            top = 20
            # 设定你想要裁剪的区域，确保不超出原图的边界
            ...
            cropped_image = image.crop(crop_box)
            # 保存或处理裁剪后的图像
            cropped_image.save('L6ZZJD-虚机存储.png')
        time.sleep(3)
        driver.close()
