...
class LDSM_run:
    def __init__(self):
        self.url = login.LDSM_url
        self.username = login.LDSM_username
        self.password = login.LDSM_password
        self.login_ldsm()
        self.captcha_ldsm()
        self.judge_ldsm()
    ...
    def captcha_ldsm(self):
        # 验证码截图保存
        captcha_element = driver.find_element(by=By.XPATH, value='//img[@id="aimg"]')
        captcha_element.screenshot('img_LDSM.png')
        ...
        # 验证码识别输入
        with open('img_LDSM.png', 'rb') as f:  # 打开图片
        ...
        while True:
            try:
                ...
                error_message = WebDriverWait(driver, 5).until(
                    EC.presence_of_element_located((By.CSS_SELECTOR, 'div[class=msg_ionc]')))
                if error_message.text == '验证码输入有误！':
                    time.sleep(2)
                    counter += 1
                    self.captcha_ldsm()  # 验证码输入错误重新输入
                elif counter == 10:
                    break
                else:
                    break
    ...
    def judge_ldsm(self):
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//div[text()='脆弱性评测']"))).click()
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//div[text()='任务列表']"))).click()
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//div[text()='虚拟机漏扫任务-Daily']"))).click()
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//span[text()='漏洞信息']"))).click()
        try:
            # 判断是否进入截图界面并截图保存
            logged_in = WebDriverWait(driver, 3).until(
                EC.presence_of_element_located((By.CSS_SELECTOR, "div[class='datagrid-cell-rownumber']")))
            if logged_in:
                time.sleep(2)
                driver.save_screenshot('word_LDSM.png')
                time.sleep(1)
                driver.close()
            else:
                print('截图失败')
