...
from selenium.common import TimeoutException, NoSuchElementException, InvalidSessionIdException
...
class IPS_run:
    def __init__(self):
        self.url = login.IPS_url
        self.username = login.IPS_username
        self.password = login.IPS_password
        self.login_IPS()
        self.captcha_IPS()
        self.judge_IPS()
    ...
    def login_IPS(self):
        ...
        user_input = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, '//input[@placeholder="请输入你的用户名"]')))
        pw_input = driver.find_element(by=By.XPATH, value='//input[@placeholder="请输入你的密码"]')
        # 输入用户名和密码，点击登录
        time.sleep(1)
    ...
    def captcha_IPS(self):
        # 验证码截图保存
        captcha_element = driver.find_element(by=By.XPATH, value='//img[@class="uedc-ppkg-login_captcha"]')
        captcha_element.screenshot('img_IPS.png')
        ...
        # 验证码识别输入
        with open('img_IPS.png', 'rb') as f:  # 打开图片
        ...
        while True:
            try:
                ...
                if error_message.text == '验证码输入不正确，请重新输入':
                    counter += 1
                    self.captcha_IPS()  # 验证码输入错误重新输入
                elif error_message.text == '请输入合法的验证码':
                    driver.close()
                    subprocess.run(r'IPS_GO.py')
                elif counter == 15:
                    driver.close()
                    break
                else:
    ...
    def judge_IPS(self):
        try:
            # 判断是否进入截图界面并截图保存
            logged_in = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "span[id='sf_user']")))
            if logged_in:
                time.sleep(3)
                driver.save_screenshot('word_IPS.png')
                time.sleep(1)
                driver.close()
            else:
                print('截图失败')
