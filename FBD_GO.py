...
class FBD_run:
    def __init__(self):
        self.url = login.FBD_url
        self.username = login.FBD_username
        self.password = login.FBD_password
        self.login_FBD()
        self.captcha_FBD()
        self.judge_FBD()
    ...
    def captcha_FBD(self):
        ...
        # 验证码识别输入
        with open('img_FBD.png', 'rb') as f:  # 打开图片
    ...
    def judge_FBD(self):
        try:
            ...
            if logged_in:
                time.sleep(3)
                driver.save_screenshot('word_FDB.png')
                time.sleep(2)
                driver.close()
            else:
                print('截图失败')
